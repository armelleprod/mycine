...
function rankLocally(heroes, alts, queryLabel) {
  ...
  const sortedHeroes = [...heroes]
    ...
    .sort((a, b) => scoreFilm(b) - scoreFilm(a));

  const hero = sortedHeroes[0] || heroes[0];

  const candidateAlts = [...alts]
    .filter(film => film.poster_path && film.id !== hero?.id)
    .sort((a, b) => scoreFilm(b) - scoreFilm(a));
  ...
  const addFromBucket = (bucket, limit) => {
    for (const film of candidateAlts) {
      if (selected.length >= 6 || limit <= 0) break;
      if (selectedIds.has(film.id)) continue;
      if (classifyFilm(film) !== bucket) continue;
  ...
  addFromBucket("mainstream", bucketTargets.mainstream);
  addFromBucket("auteur", bucketTargets.auteur);
  addFromBucket("discovery", bucketTargets.discovery);

  // Fill any gaps with the strongest remaining films, while avoiding
  // more than one pre-1990 classic in the six alternatives.
  for (const film of candidateAlts) {
    if (selected.length >= 6) break;
    if (selectedIds.has(film.id)) continue;

    const classicCount = selected.filter(item => Number(item.year) < 1990).length;
    if (Number(film.year) < 1990 && classicCount >= 1) continue;

    selected.push(film);
    selectedIds.add(film.id);
  }

  const whyWatchByType = {
    mainstream: "A polished crowd-pleaser with enough cinematic spark to rise above the algorithm.",
    auteur: "Distinctive filmmaking, strong ideas and a point of view worth your evening.",
    discovery: "A less obvious choice with the potential to become your next personal favorite."
  };

  return {
    hero: {
      i: heroes.indexOf(hero),
      whyWatch: "Tonight’s strongest meeting point between quality, accessibility and cinematic personality.",
      format: hero?.isTV ? "TV Series" : "Film",
      genres: [],
      rtCritics: Math.round((hero?.rating || 0) * 10),
      rtAudience: Math.max(0, Math.round((hero?.rating || 0) * 10 - 3))
    },
    alts: selected.map(film => ({
      i: alts.indexOf(film),
      whyWatch: whyWatchByType[classifyFilm(film)],
      format: film.isTV ? "TV Series" : "Film",
      genres: [],
      rtCritics: Math.round((film.rating || 0) * 10),
      rtAudience: Math.max(0, Math.round((film.rating || 0) * 10 - 3))
    }))
  };
}
// ── TMDB LIVE FETCH ──────────────────────────────────────────────────────────
...
async function fetchTMDBMovies(queryLabel, watchRegion) {
...
// ── MAIN PIPELINE ─────────────────────────────────────────────────────────────
async function buildPicks(tab, selGenres, selMood, watchRegion, excludeIds = []) {
...
try {
  tmdbResult = await fetchTMDBMovies(queryLabel, watchRegion);
} catch (err) {
  console.error("TMDB live request failed:", err);
  throw new Error(
    `Live TMDB request failed: ${err.message}. Check the VITE_TMDB_TOKEN in your .env file.`
  );
}
...
let alts = (tmdbResult.altMovies || [])
  ...
  // ── STEP 3: LIVE DATA ONLY ───────────────────────────────────────────────
  if (!heroes.length) {
    throw new Error(
      `TMDB returned no suitable ${CURRENT_YEAR} or ${CURRENT_YEAR - 1} films for "${queryLabel}".`
    );
  }
  ...
  const enrichProvider = async film => {
    const availability = await fetchWatchProviders(film.id, watchRegion);
    ...
    const providerNames = [...new Set(
      streamingProviders.map(item => cleanProviderName(item.provider_name))
    )];
  ...
  const enriched = await Promise.all(
    [hero, ...result].map(enrichProvider)
  );

  const availableFilms = enriched.filter(Boolean);

  if (availableFilms.length < 7) {
    throw new Error(
      `Only ${availableFilms.length} of the selected films are currently streamable in ${watchRegion}. Please try “Show me 7 more”.`
    );
  }

  return {
    hero: availableFilms[0],
    alts: availableFilms.slice(1, 7)
  };
}
...
function WhereToWatch({ movieId, region, title }) {
