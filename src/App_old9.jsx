...
async function fetchTMDBTitles(queryLabel, watchRegion, contentMode) {
  ...
  const TV_FILTERS = {
    ...
    animation: "16",
    musical: "18",
    "true stories / biopic": "18",
    documentary: "99",
    "mind blown": "10765,9648",
    "make me laugh": "35",
    "date night": "35,18",
    "travel somewhere": "10759",
    "ugly cry": "18",
    "adrenaline rush": "10759",
    "family night": "10751,16",
    "inspire me": "18",
    "luxury vibes": "18"
  };
  ...
  const mediaTypes =
    contentMode === "both" ? ["movie", "tv"] : [contentMode];

  const fetchCurrent = async mediaType => {
    let results = await requestDiscover({
      mediaType,
      year: CURRENT_YEAR,
      page: 1,
      minimumVotes: mediaType === "tv" ? 10 : 5
    });

    if (!results.length) {
      results = await requestDiscover({
        mediaType,
        year: CURRENT_YEAR - 1,
        page: 1,
        minimumVotes: mediaType === "tv" ? 20 : 20
      });
    }
...
async function buildPicks(
  ...
  try {
    tmdbResult = await fetchTMDBTitles(
      queryLabel,
      watchRegion,
      contentMode
    );
  } catch (error) {
    console.error("TMDB live request failed:", error);
    throw new Error(
      `Live TMDB request failed: ${error.message}. Check the VITE_TMDB_TOKEN in your .env file.`
    );
  }
  ...
  let heroes = (tmdbResult.heroTitles || [])
    .filter(item =>
      item.poster_path &&
      item.overview &&
  ...
  const heroIds = new Set(heroes.map(item => item.id));

  let alts = (tmdbResult.altTitles || [])
    .filter(item =>
      item.poster_path &&
      item.overview &&
      !excludeIds.includes(item.id) &&
  ...
  if (alts.length < 6) {
    throw new Error(
      `TMDB returned only ${alts.length} alternatives for "${queryLabel}". Try again or adjust the filters.`
    );
  }
  ...
  const hero = {
    ...heroRaw,
    whyWatch: ranked.hero?.whyWatch || whyTonight(heroRaw),
    format: ranked.hero?.format || heroRaw.format,
    genres: ranked.hero?.genres || [],
    rtCritics:
      ranked.hero?.rtCritics ||
      Math.round((heroRaw.rating || 0) * 10),
    rtAudience:
      ranked.hero?.rtAudience ||
      Math.max(0, Math.round((heroRaw.rating || 0) * 10 - 3))
  };
  ...
  for (const alternative of ranked.alts || []) {
    ...
    result.push({
      ...raw,
      whyWatch: alternative.whyWatch || whyTonight(raw),
      format: alternative.format || raw.format,
      genres: alternative.genres || [],
      rtCritics:
        alternative.rtCritics ||
        Math.round((raw.rating || 0) * 10),
      rtAudience:
        alternative.rtAudience ||
        Math.max(0, Math.round((raw.rating || 0) * 10 - 3))
    });
  }
  ...
  for (const raw of alts) {
    ...
    result.push({
      ...raw,
      whyWatch: whyTonight(raw),
      format: raw.format,
      genres: [],
      rtCritics: Math.round((raw.rating || 0) * 10),
      rtAudience: Math.max(0, Math.round((raw.rating || 0) * 10 - 3))
    });
  }
  ...
  const enrichTitle = async title => {
    const [details, availability] = await Promise.all([
      ...
      fetchWatchProviders(title.id, watchRegion, title.media_type)
    ]);
    ...
    const providerNames = [...new Set(
      streamingProviders.map(item => canonicalProviderName(item.provider_name))
    )];
    ...
    if (
      selectedLabels.some(label => ["romance", "romcom", "date night"].includes(label)) &&
      !inferredGenres.some(genre => genre.toLowerCase() === "romance")
    ) {
      inferredGenres.unshift("Romance");
    }

    if (
      selectedLabels.some(label => ["comedy", "romcom", "make me laugh"].includes(label)) &&
      !inferredGenres.some(genre => genre.toLowerCase() === "comedy")
    ) {
      inferredGenres.push("Comedy");
    }

    const enriched = {
      ...details,
      genres: inferredGenres.slice(0, 3),
      providerNames,
      provider: providerNames.slice(0, 3).join(" • "),
      watchLink: availability.link || null
    };

    return {
      ...enriched,
      highlight: buildHighlight(enriched),
      whyWatch: whyTonight(enriched)
    };
  };

  const enriched = await Promise.all(
    [hero, ...result].map(enrichTitle)
  );

  const availableTitles = enriched.filter(Boolean);

  if (availableTitles.length < 7) {
    throw new Error(
      `Only ${availableTitles.length} of the selected titles are currently streamable in ${watchRegion}. Please try “Show me 7 more”.`
    );
  }

  return {
    hero: availableTitles[0],
    alts: availableTitles.slice(1, 7)
  };
}
// ── POSTER COMPONENT ──────────────────────────────────────────────────────────
...
function LobbyDecor() {
  return (
    <>
      ...
      <div className="ambient-light ambient-left"/>
      <div className="ambient-light ambient-right"/>
...
function CinemaMomentCard({moment, compact=false}) {
  return (
    <article className={compact ? "projection-stage compact" : "projection-stage"}>
    ...
    <main className="page-shell">
      <div className="curator-card">
        ...
        <div className="curator-story">
          ...
          <h1>Armelle Cloche</h1>
          <p className="curator-lead">
            Hi, I’m Armelle. I believe stories can change the world, and cinema is one of the most powerful ways to tell them.
          </p>

          <p>
            I’m a screenwriter and lifelong movie lover who believes cinema is not merely entertainment. It is one of humanity’s greatest art forms.
          </p>

          <p>
            After spending far too many evenings scrolling through thousands of titles only to settle for something mediocre, I decided to build the app I wished already existed.
          </p>

          <p>
            <strong>My Ciné</strong> is my personal love letter to the Seventh Art. Instead of overwhelming you with endless choices, it curates seven thoughtful recommendations at a time, selected to help you find something genuinely worth watching.
          </p>

          <p>
            I’m offering My Ciné free because discovering a great film should be simple, joyful and shared. I hope it helps you spend less time searching and more time falling in love with cinema.
          </p>

          <div className="curator-credentials">
            <div><span>✍️</span><strong>Screenwriter</strong><small>Author of 16+ screenplays</small></div>
            <div><span>🎬</span><strong>Creator of My Ciné</strong><small>A cinematic concierge for movie lovers</small></div>
            <div><span>🎥</span><strong>A family legacy in cinema</strong><small>Granddaughter of French director Maurice Cloche</small></div>
            <div><span>❤️</span><strong>A lifelong belief</strong><small>Stories can change the world</small></div>
          </div>
...
const run = async (excludeIds = []) => {
  ...
  *{box-sizing:border-box;}
  body{margin:0;background:#5f0009;}
  ...
  .cinematic-app{
    ...
    background:
      ...
      linear-gradient(180deg,#8b0013 0%,#b0001a 34%,#7d0012 68%,#0b1f4a 100%);
  }
  ...
  .theater-valance{
    ...
    pointer-events:none;
    background:
      radial-gradient(ellipse at 8% -40%,rgba(255,70,70,0.35),transparent 42%),
      radial-gradient(ellipse at 92% -40%,rgba(255,70,70,0.35),transparent 42%),
      repeating-linear-gradient(90deg,#5d000c 0 38px,#940016 38px 78px,#63000e 78px 112px);
    clip-path:polygon(0 0,100% 0,100% 52%,96% 72%,90% 58%,84% 78%,77% 60%,70% 80%,63% 61%,56% 78%,50% 60%,44% 78%,37% 61%,30% 80%,23% 60%,16% 78%,10% 58%,4% 72%,0 52%);
  ...
  .curtain{
    ...
    background:
      ...
      repeating-linear-gradient(90deg,#4d000a 0 18px,#8f0015 18px 42px,#5e000d 42px 62px);
    box-shadow:
      inset -18px 0 30px rgba(0,0,0,0.52),
  ...
  .ambient-light{
    ...
    top:24%;
    border-radius:50%;
    background:radial-gradient(circle,rgba(255,190,92,0.16),transparent 68%);
    filter:blur(10px);
    pointer-events:none;
  }
  .ambient-left{left:7%;}
  .ambient-right{right:7%;}

  .seat-row{
    height:126px;
    opacity:0.72;
    background:
      radial-gradient(ellipse 36px 48px at 41px 62px,#6f0013 0 52%,#2d0008 55% 66%,transparent 68%) 0 0/82px 118px repeat-x,
      linear-gradient(180deg,transparent 0 45%,rgba(0,0,0,0.72) 100%);
    filter:drop-shadow(0 -7px 10px rgba(0,0,0,0.34));
  }
  .seat-row-back{bottom:116px;opacity:0.22;transform:scale(0.88);}
  .seat-row-middle{bottom:50px;opacity:0.38;transform:scale(0.95);}
  .seat-row-front{bottom:-24px;opacity:0.58;}

  /* Cinema Moments projected on a screen */
  .projection-stage{
    position:relative;
    padding:18px 18px 28px;
    isolation:isolate;
  }
  .projection-stage.compact{padding:8px 10px 24px;}
  .screen-housing{
    position:relative;
    z-index:2;
    max-width:820px;
  ...
  .screen-bottom-bar{
    ...
    border-radius:999px;
    background:
      linear-gradient(180deg,#f5d68c 0%,#9b6700 28%,#412700 55%,#c99729 82%,#6e4400 100%);
    border:1px solid rgba(255,228,160,0.58);
    box-shadow:0 3px 10px rgba(0,0,0,0.55);
  }
  .screen-bottom-bar{height:12px;}
  .projection-screen{
    position:relative;
    min-height:280px;
    overflow:hidden;
    ...
    padding:10px;
    background:#e8e0ce;
    border-left:9px solid #34220d;
    border-right:9px solid #34220d;
    box-shadow:
      inset 0 0 48px rgba(72,51,22,0.22),
      0 0 36px rgba(255,238,190,0.2);
  }
  .projection-stage.compact .projection-screen{min-height:245px;}
  .screen-content{
    position:relative;
    z-index:2;
    min-height:260px;
    ...
    align-items:center;
    text-align:center;
    padding:28px clamp(20px,5vw,62px);
    background:
      radial-gradient(circle at 50% 38%,rgba(255,255,255,0.96),rgba(243,235,214,0.92) 55%,rgba(217,204,176,0.92));
    color:#1b160f;
  }
  .projection-stage.compact .screen-content{min-height:225px;padding:24px clamp(18px,5vw,52px);}
  .screen-content .moment-kicker{color:#8f0015;}
  .screen-content h3{
    color:#15120e;
    text-shadow:none;
    margin:8px 0 10px;
  ...
  .screen-content blockquote{
    color:#2a2116;
    text-shadow:none;
    max-width:680px;
  }
  .screen-content .moment-credit{color:#7b0011;}
  .screen-content .moment-note{color:#4d4437;}
  .screen-content .moment-ribbon{
    color:#7a5a19;
    border-top-color:rgba(122,90,25,0.22);
  }
  .film-grain{
    position:absolute;
    z-index:3;
    ...
    pointer-events:none;
    background-image:
      radial-gradient(circle at 20% 30%,#000 0 0.6px,transparent 0.8px),
      radial-gradient(circle at 80% 70%,#000 0 0.6px,transparent 0.8px);
  ...
  .projector-glow{
    ...
    height:54px;
    transform:translateX(-50%);
    background:radial-gradient(ellipse,rgba(255,210,125,0.32),transparent 70%);
    filter:blur(13px);
  }
  .moments-grid{
  ...
  .curator-credentials span{
    grid-row:1/3;
    font-size:21px;
  }
  .curator-credentials strong{
    color:${C.goldBright};
    font-size:12px;
  }
  .curator-credentials small{
  ...
  @media (max-width:760px){
    ...
    .projection-stage{padding-left:0;padding-right:0;}
    .projection-screen{margin:0 8px;border-left-width:5px;border-right-width:5px;}
    .screen-content{padding:22px 16px;}
    .screen-roll,.screen-bottom-bar{margin:0 2px;}
    .theater-valance{height:72px;opacity:0.78;}
...
<div style={{display:"flex",justifyContent:"center",gap:"6px",marginBottom:"14px",flexWrap:"wrap"}}>
  {MEDIA_OPTIONS.map(option => (
    <button
      ...
      style={{
        background:contentMode===option.id?C.goldBright:C.navyMid,
        color:contentMode===option.id?C.navy:C.goldBright,
        border:`1px solid ${C.goldBright}66`,
        borderRadius:"999px",
        padding:"7px 12px",
        fontSize:"11px",
...
/>
      ...
      {page === "home" && (
      <div style={{
        background:`linear-gradient(180deg,rgba(11,31,74,0.15),rgba(11,31,74,0.92))`,
        borderTop:`1px solid ${C.goldBright}33`,
        ...
        <p className="footer-credit" style={{margin:0,color:C.white,fontFamily:"Georgia,serif"}}>
