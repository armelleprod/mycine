...
function composeStrictRomcomDatabaseBatch(
  ...
  let classicCount = Number(hero.year || 0) < 1990 ? 1 : 0;
  let eastAsianCount = isEastAsianTitle(hero) ? 1 : 0;
  let iconicCount = romcomIsIconic(hero) ? 1 : 0;
  ...
  const register = title => {
    ...
    if (Number(title.year || 0) < 1990) classicCount += 1;
    if (isEastAsianTitle(title)) eastAsianCount += 1;
    if (romcomIsIconic(title)) iconicCount += 1;
  };
  ...
  const canAdd = title => {
    if (!title || used.has(titleIdentity(title))) return false;
    if (!romcomMatchesContentMode(title, contentMode)) return false;

    if (contentMode === "both") {
      if (!title.isTV && movieCount() >= targetMovies) return false;
      if (title.isTV && tvCount() >= targetTV) return false;
    }
    ...
    if (country && (countryCount.get(country) || 0) >= 2) return false;
    if (decade && (decadeCount.get(decade) || 0) >= 2) return false;
    if (isClassic && classicCount >= 1) return false;
    if (eastAsian && eastAsianCount >= 1) return false;

    return true;
  };

  // Reserve the single Classic slot before any other role.
  // Most approved classics are American, so choosing a US crowd favorite first
  // could consume the two-title country cap and make classic-choice impossible.
  const rolePlan = [
    "classic-choice",
    "crowd-favorite",
    "passport-pick",
  ...
  for (const role of rolePlan) {
    ...
    // Before the final two slots, ensure at least two iconic choices.
    if (
      selected.length >= 3 &&
      iconicCount < 2 &&
      role !== "classic-choice"
      ...
      ranked = [
        ...ranked.filter(romcomIsIconic),
        ...ranked.filter(title => !romcomIsIconic(title))
      ];
    }

    const chosen = ranked.find(canAdd);

    if (!chosen) {
      throw new Error(`No strict Romcom candidate available for ${role}.`);
    }
  ...
  const titles = [hero, ...selected];

  if (titles.length !== 7) {
    throw new Error("Strict Romcom batch did not contain seven titles.");
  }

  if (iconicCount < 2) {
    throw new Error("Strict Romcom batch did not include two iconic choices.");
  }

  if (classicCount > 1) {
    throw new Error("Strict Romcom batch contained more than one classic.");
  }

  if (eastAsianCount > 1) {
    throw new Error("Strict Romcom batch contained more than one East Asian title.");
  }

  if (contentMode === "movie" && titles.some(title => title.isTV)) {
    throw new Error("A TV series entered a Movies-only Romcom batch.");
  }

  if (contentMode === "tv" && titles.some(title => !title.isTV)) {
    throw new Error("A film entered a TV-only Romcom batch.");
  }
      ...
      <div className="app-content">
        {error&&(
          <div className="public-error">
            <strong>🎬 My Ciné could not compose the next strict Rule-of-Seven Romcom set.</strong>
