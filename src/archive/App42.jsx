...
// ── MY CINÉ ENGINE 2.0 ───────────────────────────────────────────────────────
const RECOMMENDATION_HISTORY_KEY = "mycine-recommendation-history-romcom-db-v6";
...
const ROMCOM_USED_LEDGER_KEY = "mycine-romcom-used-ledger-v1";
...
async function buildIncrementalRomcomBatch(
  ...
  const [rawCanon, rawArrivals] = await Promise.all([
    fetchCanonCandidates(
      "romcom",
      contentMode,
      [...usedIds]
    ),
    fetchNewArrivalCandidates(
      "romcom",
      contentMode,
      [...usedIds]
    )
  ...
  const normalizeUnused = items =>
    items
      .map(normalizeDatabaseCandidate)
      .filter(title => !usedIds.has(String(title.id)));

  const canonRaw = normalizeUnused(rawCanon);
  const arrivalsRaw = normalizeUnused(rawArrivals);

  // Resolve recent arrivals first, then progressively resolve Canon titles
  // until enough strong choices exist.
  const recentResolved = (
    await mapWithConcurrency(
      arrivalsRaw.slice(0, 24),
      title => enrichRomcomDatabaseTitle(title, watchRegion),
      4
    )
  ).filter(Boolean);
  ...
  for (
    let start = 0;
    start < canonRaw.length && canonResolved.length < 18;
    start += chunkSize
  ) {
    const chunk = canonRaw.slice(start, start + chunkSize);
    ...
    canonResolved.push(...resolvedChunk);
  }
  ...
  const finalHero =
    hero ||
    chooseRomcomPlanHero([], dedupedCanon, new Set());
  ...
  const alternatives = composeRomcomPlanAlternatives(
    finalHero,
    dedupedCanon,
    new Set([titleIdentity(finalHero)]),
    contentMode,
    batchNumber
  );

  const titles = [finalHero, ...alternatives];

  if (
    titles.length !== 7 ||
    new Set(titles.map(titleIdentity)).size !== 7
  ) {
    throw new Error("The incremental Romcom batch was not seven unique titles.");
  }
      ...
      <div className="app-content">
        {error&&(
          <div className="public-error">
            <strong>🎬 My Ciné could not resolve seven unused Romcom choices this time.</strong>
