...
// ── MY CINÉ ENGINE 2.0 ───────────────────────────────────────────────────────
const RECOMMENDATION_HISTORY_KEY = "mycine-recommendation-history-romcom-db-v1";
...
async function buildPicks(
  ...
  const alternatives = selectedLabels.includes("romcom")
    ? assembleRomcomDatabaseBatch(
        hero,
        candidates,
        contentMode,
        batchNumber
...
const run = async (
  ...
  const doFetch = () => {
    ...
    // Keep the current seven visible until the replacement batch succeeds.
      ...
      <div className="app-content">
        {error&&(
          <div className="public-error">
            <strong>🎬 My Ciné is preparing a stronger fresh set.</strong>
            <span>Your current seven will remain here while the next selection is prepared.</span>
          </div>
        )}
        {loading&&<Skeleton/>}
