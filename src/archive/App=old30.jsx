...
function assembleEngineBatch(hero, candidates, selectedLabels, contentMode, batchNumber) {
  ...
  if (selected.length < remainingSlots) {
    throw new Error("The My Ciné Engine could not build a balanced set of seven from the qualified titles.");
  }
...
async function fetchTMDBTitles(queryLabel, watchRegion, contentMode) {
  ...
  const matchesLabel = (label, item, mediaType) => {
    ...
    switch (label) {
      case "romcom": return genreIds.has(35) && (isMovie ? genreIds.has(10749) : textMatches(titleText, romanceWords));
      case "comedy":
  ...
  const heroPreviousRequests = mediaTypes.flatMap(mediaType => [
    requestDiscover({
      ...
      year: CURRENT_YEAR - 1,
      page: 1,
      minimumVotes: mediaType === "tv" ? 25 : 35,
      sortBy: "vote_average.desc"
    }),
    requestDiscover({
      mediaType,
...
async function buildPicks(
  ...
  if (!heroCandidates.length || altCandidates.length < 12) {
  ...
  const enrichTitle = async title => {
    ...
    const providerNames = [...new Set(
      providers.map(item => canonicalProviderName(item.provider_name))
    )];

    if (!providerNames.length || !availability.link) return null;
  ...
  const enrichedHeroes = (await Promise.all(preRankedHero.map(enrichTitle))).filter(Boolean);
  const enrichedAlts = (await Promise.all(preRankedAlts.map(enrichTitle))).filter(Boolean);
