...
const MOOD_ENGINE_CONFIG = {
  mindblow:{
    label:"Mind Blown",
    tags:["mind blown","surprise me"],
    genres:["scifi","mystery","thriller","fantasy","animation"],
    note:"A bold, inventive choice designed to rearrange your cinematic furniture."
  },
  laugh:{
    label:"Make Me Laugh",
    tags:["laugh","feel good","surprise me"],
    genres:["comedy","romcom","animation","musical"],
    note:"A genuinely funny choice selected to brighten the room."
  },
  date:{
    label:"Date Night",
    tags:["date night","feel good"],
    genres:["romcom","romance","comedy","lgbtq","musical"],
    note:"A warm, engaging choice with enough spark for two."
  },
  feelgood:{
    label:"Feel Good",
    tags:["feel good","family night","escape"],
    genres:["comedy","romcom","animation","musical","romance"],
    note:"An uplifting choice selected to leave the evening lighter."
  },
  cry:{
    label:"Ugly Cry",
    tags:["ugly cry","heal my heart"],
    genres:["drama","romance","biopic","lgbtq","animation"],
    note:"An emotionally generous story chosen for a beautiful, cathartic cry."
  },
  think:{
    label:"Make Me Think",
    tags:["make me think","mind blown"],
    genres:["drama","documentary","biopic","scifi","mystery","international"],
    note:"A layered choice selected to keep the conversation alive after the credits."
  },
  inspire:{
    label:"Inspire Me",
    tags:["inspire me","feel good","make me think"],
    genres:["biopic","documentary","drama","animation","international"],
    note:"A stirring story about courage, imagination or meaningful change."
  },
  travel:{
    label:"Travel Somewhere",
    tags:["travel somewhere","escape"],
    genres:["international","fantasy","adventure","romance"],
    note:"A transporting choice selected to take you somewhere memorable."
  },
  escape:{
    label:"Escape Reality",
    tags:["escape","surprise me"],
    genres:["fantasy","scifi","animation","action","musical"],
    note:"An immersive world chosen to make ordinary reality briefly lose your address."
  },
  adrenaline:{
    label:"Adrenaline Rush",
    tags:["adrenaline","mind blown"],
    genres:["action","thriller","horror","scifi"],
    note:"A propulsive choice selected to keep the pulse pleasantly busy."
  },
  chills:{
    label:"Give Me Chills",
    tags:["adrenaline","mind blown"],
    genres:["horror","thriller","mystery"],
    note:"An atmospheric choice designed to send a small, elegant shiver down the spine."
  },
  family:{
    label:"Family Night",
    tags:["family night","feel good"],
    genres:["animation","fantasy","comedy","musical"],
    note:"A welcoming choice selected for shared viewing across generations."
  },
  nostalgia:{
    label:"Nostalgia Trip",
    tags:["nostalgia","feel good"],
    genres:["animation","comedy","musical","romance","fantasy"],
    note:"A comforting choice with the glow of remembered movie nights."
  },
  mystery:{
    label:"Solve the Mystery",
    tags:["mind blown","make me think"],
    genres:["mystery","thriller","crime"],
    note:"A clue-rich choice selected for viewers who enjoy outsmarting the screen."
  },
  heal:{
    label:"Heal My Heart",
    tags:["heal my heart","ugly cry","feel good"],
    genres:["drama","romance","romcom","lgbtq","animation"],
    note:"A tender choice selected to restore a little faith in people and possibility."
  },
  luxury:{
    label:"Luxury Vibes",
    tags:["luxury vibes","escape","date night"],
    genres:["romance","drama","international","biopic","romcom"],
    note:"A polished choice filled with style, glamour, beautiful spaces or delicious excess."
  }
};

const MOOD_ROLE_BADGES = [
  "🍿 Tonight's Pick",
  "❤️ Crowd Favorite",
  "🎞️ Classic",
  "🌍 Passport",
...
function normalizedMoodTags(entry) {
  return (entry.moodTags || [])
    .map(tag => String(tag).trim().toLowerCase())
    .filter(Boolean);
}

function moodSourceCatalog() {
  return [
    {genre:"romcom", movies:ROMCOM_EDITORIAL_BATCHES, series:ROMCOM_SERIES_EDITORIAL_BATCHES},
    {genre:"comedy", movies:COMEDY_EDITORIAL_BATCHES, series:COMEDY_SERIES_EDITORIAL_BATCHES},
    {genre:"romance", movies:ROMANCE_EDITORIAL_BATCHES, series:ROMANCE_SERIES_EDITORIAL_BATCHES},
    {genre:"drama", movies:DRAMA_EDITORIAL_BATCHES, series:DRAMA_SERIES_EDITORIAL_BATCHES},
    {genre:"thriller", movies:THRILLER_EDITORIAL_BATCHES, series:THRILLER_SERIES_EDITORIAL_BATCHES},
    {genre:"mystery", movies:MYSTERY_EDITORIAL_BATCHES, series:MYSTERY_SERIES_EDITORIAL_BATCHES},
    {genre:"action", movies:ACTION_ADVENTURE_EDITORIAL_BATCHES, series:ACTION_ADVENTURE_SERIES_EDITORIAL_BATCHES},
    {genre:"horror", movies:HORROR_EDITORIAL_BATCHES, series:HORROR_SERIES_EDITORIAL_BATCHES},
    {genre:"scifi", movies:SCIFI_EDITORIAL_BATCHES, series:SCIFI_SERIES_EDITORIAL_BATCHES},
    {genre:"fantasy", movies:FANTASY_EDITORIAL_BATCHES, series:FANTASY_SERIES_EDITORIAL_BATCHES},
    {genre:"animation", movies:ANIMATION_EDITORIAL_BATCHES, series:ANIMATION_SERIES_EDITORIAL_BATCHES},
    {genre:"musical", movies:MUSICAL_EDITORIAL_BATCHES, series:MUSICAL_SERIES_EDITORIAL_BATCHES},
    {genre:"biopic", movies:BIOPIC_EDITORIAL_BATCHES, series:BIOPIC_SERIES_EDITORIAL_BATCHES},
    {genre:"documentary", movies:DOCUMENTARY_EDITORIAL_BATCHES, series:[]},
    {genre:"international", movies:INTERNATIONAL_EDITORIAL_BATCHES, series:INTERNATIONAL_SERIES_EDITORIAL_BATCHES},
    {genre:"hiddengems", movies:HIDDEN_GEMS_EDITORIAL_BATCHES, series:HIDDEN_GEMS_SERIES_EDITORIAL_BATCHES},
    {genre:"lgbtq", movies:LGBTQ_EDITORIAL_BATCHES, series:LGBTQ_SERIES_EDITORIAL_BATCHES}
  ];
}

function flattenMoodEntries(catalog, mediaType) {
  const unique = new Map();

  catalog.forEach(source => {
    const batches = mediaType === "tv" ? source.series : source.movies;

    (batches || []).forEach((batch, batchIndex) => {
      batch.forEach((entry, slotIndex) => {
        const prepared = {
          ...entry,
          type:mediaType,
          sourceGenre:source.genre,
          sourceBatch:batchIndex + 1,
          sourceSlot:slotIndex + 1
        };
        const key = [
          mediaType,
          String(prepared.title || "").toLowerCase(),
          prepared.year || ""
        ].join("|");
...
function moodEntryScore(entry, moodId) {
  const config = MOOD_ENGINE_CONFIG[moodId] || MOOD_ENGINE_CONFIG.feelgood;
  const tags = normalizedMoodTags(entry);
  const aliases = config.tags.map(tag => tag.toLowerCase());

  let score = 0;
  aliases.forEach(alias => {
    if (tags.includes(alias)) score += 80;
    else if (tags.some(tag => tag.includes(alias) || alias.includes(tag))) score += 45;
  });

  const genreIndex = config.genres.indexOf(entry.sourceGenre);
  if (genreIndex >= 0) score += 42 - (genreIndex * 4);

  if (entry.sourceGenre === "hiddengems") score += 8;
  if (entry.badge === "🏆 Critics") score += 8;
  if (entry.badge === "❤️ Crowd Favorite") score += 6;
  if (entry.badge === "✨ Surprise") score += 5;

  return score;
}

function rankedMoodPool(mediaType, moodId) {
  const config = MOOD_ENGINE_CONFIG[moodId] || MOOD_ENGINE_CONFIG.feelgood;
  const entries = flattenMoodEntries(moodSourceCatalog(), mediaType);

  return entries
    .map(entry => ({
      entry,
      score:moodEntryScore(entry, moodId),
      tie:stableMoodHash(`${moodId}|${mediaType}|${entry.title}|${entry.year}`)
    }))
...
function takeMoodEntry(pool, used, preferredGenre = null) {
  const preferredIndex = preferredGenre
    ? pool.findIndex(entry =>
        entry.sourceGenre === preferredGenre &&
        !used.has(`${entry.type}|${entry.title}|${entry.year}`)
      )
    : -1;

  const fallbackIndex = pool.findIndex(
    entry => !used.has(`${entry.type}|${entry.title}|${entry.year}`)
  );
  const selectedIndex = preferredIndex >= 0 ? preferredIndex : fallbackIndex;

  if (selectedIndex < 0) return null;
  const [selected] = pool.splice(selectedIndex, 1);
  used.add(`${selected.type}|${selected.title}|${selected.year}`);
  return selected;
}

function buildMoodEditorialBatches(moodId, contentMode) {
  const config = MOOD_ENGINE_CONFIG[moodId] || MOOD_ENGINE_CONFIG.feelgood;
  const moviePool = rankedMoodPool("movie", moodId);
  const tvPool = rankedMoodPool("tv", moodId);
  const used = new Set();

  return Array.from({length:7}, (_, batchIndex) => {
    const movieLeads = batchIndex % 2 === 0;

    return MOOD_ROLE_BADGES.map((badge, slotIndex) => {
      const preferredGenre =
        config.genres[(batchIndex + slotIndex) % config.genres.length];

      let pool;
      if (contentMode === "movie") pool = moviePool;
      else if (contentMode === "tv") pool = tvPool;
      else {
        const useMovie = movieLeads
          ? slotIndex % 2 === 0
          : slotIndex % 2 === 1;
        pool = useMovie ? moviePool : tvPool;
      }

      let selected = takeMoodEntry(pool, used, preferredGenre);

      if (!selected && contentMode === "both") {
        selected = takeMoodEntry(
          pool === moviePool ? tvPool : moviePool,
          used,
          preferredGenre
        );
      }
      ...
      if (!selected) {
        throw new Error(
          `The ${config.label} mood pool could not prepare all seven recommendations.`
        );
      }
      ...
      return {
        ...
        role:badge,
        slot:slotIndex + 1,
        curatorNote:config.note,
        moodTags:[
          ...new Set([
            ...normalizedMoodTags(selected),
            ...config.tags
          ])
        ],
        moodEngine:true,
        balancedBoth:contentMode === "both"
      };
    });
  });
}
...
export default function App() {
  ...
  const activePreparedBatches =
    tab === "mood" && selMood
      ? buildMoodEditorialBatches(selMood, contentMode)
...
const run = async (
  ...
  try {
    const preparedGenre =
      tab === "mood" && selMood && Boolean(activePreparedBatches)
        ? `mood-${selMood}`
        : tab === "genre" &&
            selGenres.length === 1 &&
