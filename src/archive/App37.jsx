...
async function buildPicks(
  ...
  let altCandidates = rawAlternativePool
    .map(normalizeTitle)
    .filter(title =>
      title.rating >= 7.5 &&
  ...
  // Never bypass freshness for the Romcom Canon.
  // Other unfinished genres keep the older fallback temporarily.
  if (!selectedLabels.includes("romcom") && altCandidates.length < 28) {
    altCandidates = rawAlternativePool
      .map(normalizeTitle)
      .filter(title => title.rating >= 7.5 && notShownInSession(title));
  }
    ...
    <main className="page-shell">
      ...
      <div className="standard-grid">
        ...
        <section className="standard-card">
          ...
          <p>Every recommendation must reach at least 75% in the live TMDB community rating. The Romcom Canon now contains 100 editorially approved titles. Alternatives come only from that library, while TMDB supplies live ratings, posters and availability.</p>
...
const run = async (
  ...
  try {
    ...
    setError("My Ciné could not prepare the next fresh set yet. Please try Keep Exploring again.");
  } finally {
    setLoading(false);
    clearInterval(timer);
  }
