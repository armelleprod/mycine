...
import {ROMCOM_NEW_ARRIVALS} from "./romcomArrivals";
...
export function getCanonEntriesForGenre(genre) {
  return ALL_CANON_ENTRIES.filter(entry =>
    entry.status === "approved" &&
    (entry.primaryGenre === genre || entry.secondaryGenres.includes(genre))
  );
}

export function getNewArrivalsForGenre(genre, contentMode = "both") {
  if (genre !== "romcom") return [];

  return ROMCOM_NEW_ARRIVALS.filter(entry =>
    contentMode === "both" ||
...
export const CANON_STATS = Object.freeze({
  ...
  romcomNewArrivals:ROMCOM_NEW_ARRIVALS.length
});
